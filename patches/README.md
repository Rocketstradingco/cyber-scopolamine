...
2. **`/model` picker:** on this offline, no-API-key box, `/model`'s
   ...
   instead of the 4 locally-configured Ollama aliases — so pressing Tab
   ...
   + Tab (even with nothing typed yet) arrow-completes through exactly
   the local Ollama-backed aliases, and `/model` with no argument prints
...
- `0003-commands-py.patch` — adds `Commands._local_model_aliases()`
  ...
  `completions_model` (Tab-completion candidates) so only real local
  options are ever offered — the cloud catalog is still reachable via
...
- `apply.sh` — locates the active `aider` install via `which aider` and
  applies all four patches with `patch -p1`.
...
intact. Type `/model` with no argument to confirm only the 4 local
aliases show (not sonnet/opus/haiku/etc.), then type `/model ` and press
Tab to confirm the same 4 show up as an arrow-navigable dropdown with
nothing typed yet.
